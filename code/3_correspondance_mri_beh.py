...
rawdata_dir = "/media/francoisramon/CurrentDisk/APEX/apex_data/rawdata_bids" ### IRM



subject_list = [s for s in os.listdir(rawdata_dir) if "sub-ado" in s and not "." in s]
subject_list_beh = [s for s in os.listdir(rawdata_dir) if "sub-ado" in s and not "." in s]
...
# 			tsv_filelist = glob.glob(os.path.join(rawdata_dir,sub,ses,"beh","*.tsv"))
...
# 					T1_file = glob.glob(os.path.join(rawdata_dir,raw_subject_id,ses,"anat","*_T1w.json"))[0]
...
## ne peux pas fonctionner car pas la date dans les données IRM (uniquement l'heure)
...
if unicity:
	for sub in tqdm(subject_list_beh):
		...
		session_list = [s for s in os.listdir(os.path.join(rawdata_dir,sub)) if "ses" in s and '.' not in s and not "training" in s]
		...
		for ses in session_list:
			...
			list_tsv = glob.glob(f"/media/francoisramon/CurrentDisk/APEX/apex_data/rawdata_bids/{sub}/{ses}/beh/*_beh.tsv")
...
sub-adoca024 - ses-pre : multiple dates
[datetime.datetime(2016, 10, 20, 0, 0), datetime.datetime(2016, 10, 21, 0, 0)]
#1j d'écart OK

sub-adoci002 - ses-post : multiple dates
[datetime.datetime(2016, 11, 8, 0, 0), datetime.datetime(2016, 6, 28, 0, 0)]
# 133 jours !!
# check sub-adoci002 pre --> 24 mai (quand meme 34 jours)

sub-adoci034 - ses-pre : multiple dates
[datetime.datetime(2017, 2, 16, 0, 0), datetime.datetime(2017, 2, 8, 0, 0)]
# 8j d'écart OK?



sub-adopc023 - ses-post : multiple dates
[]
